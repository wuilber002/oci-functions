...
This set of files contains a ***Function*** in Python developed to automate the process of *downloading* and *uploading* the recommendations, categories and actions recommended by the **Cloud Advisor** from your *Tenancy* OCI to a *bucket* within the same *tenancy*.

<h2> Disclaimer </h3>

Before proceeding, it is essential to bear in mind that the use of any *scripts*, codes or commands contained in this repository is at your own risk. The authors of the codes are not responsible for any charges arising from the use of the content made available here.
...
The *script* in Python, designed to be executed in the *serverless* **OCI Functions** service, extracts data from the OCI API using the following calls:
...
After extracting the lists of categories and recommendations, the *script* uses an external mapping file (*mapping*/dictionary) to translate the programmatic values (*keys*) returned by the API in the name and description fields into **humanized** (user-readable) values.

This mapping file needs to be made **available on the same bucket** used by OCI Function as the destination for the extracted files. More information will be provided throughout this document.

> [!WARNING]
**It is mandatory that the mapping file is periodically validated. New categories or recommendations made available by Oracle **will not be translated automatically** and need to be manually inserted into the dictionary to ensure the accuracy of the data.
...
- [Resource Scheduler](#resource-scheduler)
  - [Dynamic Scheduling Criteria](#dynamic-scheduling-criteria)
...
To continue with this procedure, you must **fulfill the following requirements**:
...
It is mandatory that you have a **VCN** created with a **Subnet** that has IP addresses available to allocate the *Function*. This Subnet must have Internet access or have a Service Gateway active.

## Project files

Below are the files that make up the project. Only two are essential; `readme.md` and `readme_en-US` can be ignored in the deployment process.
...
|**func.py**|The *script* in Python that will be executed by the *function*.|
|**cloud-advisor-mapping.json**|Mapping file (dictionary) for the names and descriptions of the *Cloud Advisor* recommendations and categories.|
...
|**OCI_DOMAIN_NAME** |Name of the domain in which the user used is created.|
...
|**OCI_BUCKET_ROOT_PATH** |Name of the **root directory** of the destination bucket for the Cloud Advisor files.|
|**CLOUD_ADVISOR_MAPPING_FILE** |Name of the Cloud Advisor mapping file.|
|**CLOUD_ADVISOR_MAPPING_FILE_PATH**| Path to the Cloud Advisor mapping file.|
...
| **OCI_TENANCY** |OCID (*Oracle Cloud Identifier*) of the *Tenancy* to which we are logged into the *Cloud Shell*.|
...
Clone this git repository or make all the files mentioned in the [Project files](#project-files) topic available in our working directory.
...
It is essential to use an **Auth Token as a password** during the *login* process in the **OCI Container Registry**.

If you don't have an *Auth Token* created, generate a new one using the following command:
...
> [!TIP]
> In case you have just created the **Auth Token**, **wait a few minutes** before using it. It may take a while for the token to propagate and be ready to log into the OCI Container Registry.
...
This *Bucket* will receive all the files extracted from the **Cloud Advisor** service for *Tenancy*.

If you already have the *Bucket* created, you can skip this part and export the **OCI_BUCKET_NAME_DESTINATION** variable with the name of your bucket. Otherwise, create the bucket with the command below:
...
Now let's upload the *mapping* file to the bucket in the **PATH** specified at the beginning of this procedure in the **${CLOUD_ADVISOR_MAPPING_FILE_PATH}** environment variable.

Use the command below to upload the mapping file to the bucket we have just created:
...
|CLOUD_ADVISOR_MAPPING_FILE_PATH|Complete path within the *Bukcet* to the data *mapping* file, which maps the names and descriptions of the *Cloud Advisor* recommendations and categories.|
...
- Create the repository in the **OCI Container Registry** for the *upload* of the *image*.
...
To **grant the permissions** necessary for the *function* to access the *Cloud Advisor* service and the *target bucket*, it is essential to create a **Dynamic Group**, according to the command below:
...
--name ${DYG_FUNCTION} \
--description "Dynamic group for the function that extracts the Cloud Advisor recommendations." \
--matching-rule "ALL {resource.type = 'fnfunc', resource.id = '${FN_FUNC_OCID}'}"
```

> [!NOTE]
The **`matching-rule``** of this **Dynamic Group** will exclusively select our *function*, regardless of the *compartment* of creation.

### Policy for Function

This access policy will grant the *function* the ability to query the Cloud Advisor API and the target *bucket*, using the dynamic group created.
...
--description "Permissions for the function that extracts the Cloud Advisor recommendations." \
...
> [!NOTE]
If you have a pre-existing schedule that you want to reuse, please proceed to the topic: [Using existing scheduling](#using-existing-scheduling)

---

The schedule to be created will be of the **dynamic** type, configured for **one daily run** at the following time:
...
### Dynamic Scheduling Criteria

A dynamic schedule selects the resources to be executed at the time of the invocation, based on previously defined filter criteria. We will adopt the following criteria for this *function*:

- **Compartment:** Reside in the **Compartment** defined by the `OCI_COMPARTMENT` environment variable.
- **Free-form Tag:** Have the **Free-form Tag** with the exact key and value: `FinOps: Scheduled-Function`.
- **Resource type:** Be a resource of type **Function** (OCI Function).

Based on the criteria listed, we will now proceed to create the **Resource Filter** JSON file, which will be used to identify the resources that will have their execution scheduled.
...
| Item | Description |
|-|-|
|categories_names |Quantity of **category names** changed by field mapping. |
|categories_descriptions |Quantity of **category descriptions** changed by field mapping.      |
|recommendations_names |Quantity of names of **recommendations** changed by field mapping. |
|recommendations_descriptions |Quantity of descriptions of **recommendations** changed by field mapping. |
|categories_total |Total quantity of names of **categories** processed by field mapping. |
|recommendations_total |Total quantity of names of **recommendations** processed by field mapping.|

## Logging

In the event of problems or errors, **keep a *log* and follow the events** in order to identify and correct any faults.
...
> IMPORTANT]
Function logging is not enabled by default. If necessary, you need to activate it manually.
